# 阅可（readmigo-cn-repos）

> **项目代号**：`readmigo-cn-repos`
> **状态**：📋 立项 · 待启动 Phase 0
> **平台**：HarmonyOS NEXT 5.0+（纯血鸿蒙）
> **位置**：`/Users/HONGBGU/Documents/readmigo-cn-repos/`（与 `readmigo-repos/` 平级独立）
>
> ⚠️ **本项目和 Readmigo 海外版（readmigo-repos）是两个完全独立的项目**，**互相无关，单独发展**。无 git submodule，无自动同步。初期可拷贝部分代码起步，拷贝后两边独立演化。

---

## 项目是什么

面向中国大陆华为终端用户的**纯血鸿蒙原生**英文阅读 + AI 学习产品，**完整拥抱**鸿蒙生态：
...
**两个独立项目 · 平级 · 无从属关系 · 无自动同步**

| 维度 | 海外版（readmigo-repos） | 国内版（本项目） |
|------|------|------|
| 主体 | 海外公司 | 境内公司 |
| 代码 | GitHub | Gitee + 极狐 GitLab |
| 数据 | 海外 PostgreSQL | 华为云 GaussDB |
| LLM | OpenAI / Claude | DeepSeek / 文心 / 智谱 / Qwen |
| 商店 | App Store / Google Play | 华为应用市场 |
| 域名 | readmigo.app | readmigo.cn（待定）|
| 路线图 | 海外独立 | 国内独立 |

**初期参考关系**：Phase 1 可从海外版拷贝部分代码起步（C++ 引擎 / 后端核心 / 数据集），**拷贝后两边各自演化、互不影响**。无 git submodule，无自动同步。
