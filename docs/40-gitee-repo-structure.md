...
- ✅ [typesetting](https://gitee.com/readmigo/typesetting) (Private)
- ✅ [badge-engine](https://gitee.com/readmigo/badge-engine) (Private)
...
```bash
cd /tmp
git clone git@gitee-readmigo-cn:readmigo/readmigo-cn-repos.git extract-harmony-app
...
git remote add origin git@gitee-readmigo-cn:readmigo/harmony-app.git
...
# 改 README / PROJECT.md / docs/ 中所有指向 harmony-app/ 的路径 → 改成 gitee.com/readmigo/harmony-app URL
...
```bash
cd /tmp
git clone git@gitee-readmigo-cn:readmigo/readmigo-cn-repos.git extract-docs
cd extract-docs
...
cd /tmp/extract-docs
git remote add origin git@gitee-readmigo-cn:readmigo/docs-cn.git
git push -u origin main
```
