...
|---------|------|------|----------|---------|-------|------|
| `api` | 118,855 | TypeScript | `server-cn/api/` ([github](https://github.com/readmigo-cn/server-cn/blob/main/src/modules/ai/)) | 重写 LLM provider + 接 GaussDB + 国行登录 | 70% | 3-4 周 |
| `gutenberg` | 35,314 | JSON | `server-cn/content/gutenberg-cn/` ([github](https://github.com/readmigo-cn/server-cn/blob/main/content/gutenberg-cn/)) | 白名单过滤 + 内容审核 | 60% | 1 周 |
| `tts-data` | 9,712 | JSON | `server-cn/tts-data-cn/` ([github](https://github.com/readmigo-cn/server-cn/blob/main/tts-data-cn/)) | 重新用讯飞 TTS 生成 | 50% | 1 周 |
| `tts` | 17,635 | JSON | `server-cn/tts/` ([github](https://github.com/readmigo-cn/server-cn/blob/main/tts/)) | 接讯飞 TTS API | 70% | 1 周 |
| `ocr-pipeline` | 454 | TypeScript | `server-cn/ocr-pipeline/` ([github](https://github.com/readmigo-cn/server-cn/blob/main/ocr-pipeline/)) | 接百度 OCR API | 90% | 0.5 周 |
| `nlp` | 299 | Python | `server-cn/nlp/` ([github](https://github.com/readmigo-cn/server-cn/blob/main/nlp/)) | 接 DeepSeek / 文心 | 80% | 0.5 周 |
| `ai` | 2,031 | TypeScript | `server-cn/ai/` ([github](https://github.com/readmigo-cn/server-cn/blob/main/ai/)) | 重写 LLM provider 层 | 60% | 1 周 |
| `audiolab` | 3,776 | TypeScript | `server-cn/audiolab/` ([github](https://github.com/readmigo-cn/server-cn/blob/main/audiolab/)) | 接讯飞 TTS | 85% | 1 周 |
| `book-translator` | 1,177 | TypeScript | `server-cn/book-translator/` ([github](https://github.com/readmigo-cn/server-cn/blob/main/book-translator/)) | 接百度翻译 | 80% | 0.5 周 |
...
### 4.1 `server-cn/api` ([github](https://github.com/readmigo-cn/server-cn/blob/main/api))（NestJS 后端）
...
### 4.2 `server-cn/ocr-pipeline` ([github](https://github.com/readmigo-cn/server-cn/blob/main/ocr-pipeline))

- [ ] 替换 OCR provider：tesseract / Azure → **百度智能云 OCR**
- [ ] 内容审核：增加白名单过滤
- [ ] 部署到华为云

### 4.3 `server-cn/audiolab` ([github](https://github.com/readmigo-cn/server-cn/blob/main/audiolab)) + `server-cn/tts` ([github](https://github.com/readmigo-cn/server-cn/blob/main/tts))
