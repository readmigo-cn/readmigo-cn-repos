# 阅可（readmigo-cn-repos）项目主文件

> **项目代号**：`readmigo-cn-repos`
> **中文产品名**：阅可（暂定，待商标确认）
> **创建日期**：2026-04-26
> **当前状态**：📋 立项 / 主文件就位 / 待资源到位启动 Phase 0
> **位置**：`/Users/HONGBGU/Documents/readmigo-cn-repos/`（与 `readmigo-repos/` **平级独立**，不存在从属关系）
>
> ⚠️ **重要：本项目和 Readmigo 海外版（`readmigo-repos/`）是两个完全独立的项目**：
> - **法律主体独立** / **数据隔离** / **代码独立维护** / **路线图独立演化**
> - **不使用 git submodule，不做自动同步**
> - 初期可**拷贝部分代码**（C++ 引擎 / 后端核心 / 数据集），拷贝后**两边各自演化、互不影响**
> - 国内项目修改不会反向影响海外版，反之亦然
...
| **不是什么** | ❌ 不是 Readmigo 海外版的鸿蒙端 ❌ 不是简单移植 ❌ 不是国际化产品 |
...
```
┌────────────────────┐         ┌────────────────────┐
│  Readmigo 海外版    │  完全   │  阅可（国内版）     │
│  readmigo-repos/   │  独立   │  readmigo-cn-repos/│
│                    │  无关   │                    │
│  ✓ 海外公司         │  各自   │  ✓ 境内公司         │
│  ✓ GitHub          │  发展   │  ✓ Gitee / 极狐     │
│  ✓ Cloudflare/Fly  │         │  ✓ 华为云 / 阿里云  │
│  ✓ OpenAI/Claude   │         │  ✓ DeepSeek/文心    │
│  ✓ App Store/GP    │         │  ✓ 华为应用市场      │
│  ✓ readmigo.app    │         │  ✓ readmigo.cn      │
└────────────────────┘         └────────────────────┘

无 git submodule / 无自动同步 / 无双向反馈
仅初期可参考海外版代码拷贝起步，之后各自演化
```

**关键原则：法律隔离 + 数据隔离 + 代码隔离 + 团队隔离 + 路线图隔离**
