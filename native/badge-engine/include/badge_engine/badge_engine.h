...
/* Load model from memory buffer (GLB format, no .badge file needed) */
int badge_engine_load_model_data(BadgeEngine* engine, const uint8_t* glb_data, uint32_t data_size);

/* Set texture for a specific face (RGBA8888 pixel data) */
...
/* Set material parameters for a specific face (JSON string of MaterialParams) */
