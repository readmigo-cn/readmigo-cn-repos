...
int Engine::loadBadge(const std::string& badge_path) {
    ...
    // If badge_path is an already-extracted directory (contains manifest.json),
    // use it directly. Otherwise unzip .badge package to temp directory.
    namespace fs = std::filesystem;
    std::string extract_dir;
    if (fs::is_directory(badge_path) && fs::exists(badge_path + "/manifest.json")) {
        extract_dir = badge_path;
    } else {
        extract_dir = "/tmp/badge_engine_" + std::to_string(std::hash<std::string>{}(badge_path));
    }
...
int Engine::loadModelData(const uint8_t* glb_data, uint32_t data_size) {
    if (!surface_ready_ || !glb_data || data_size == 0) return -1;
...
#ifdef BADGE_ENGINE_HAS_FILAMENT
    if (!scene_->loadModelFromMemory(glb_data, data_size, target_lod)) {
        return -1;
    }
...
int Engine::setFaceTexture(int face, const uint8_t* rgba_data, uint32_t width, uint32_t height) {
    if (!badge_loaded_ || !rgba_data) return -1;

#ifdef BADGE_ENGINE_HAS_FILAMENT
    return scene_->setFaceTexture(face, rgba_data, width, height) ? 0 : -1;
#else
    return -1;
#endif
}

int Engine::setFaceMaterial(int face, const char* material_json) {
    if (!badge_loaded_ || !material_json) return -1;

    auto params = material_system_->parseJSON(std::string(material_json));
    if (!params.has_value()) return -1;

#ifdef BADGE_ENGINE_HAS_FILAMENT
    return scene_->setFaceMaterial(face, params.value()) ? 0 : -1;
#else
    return -1;
#endif
}
