...
public:
    ...
    int loadModelData(const uint8_t* glb_data, uint32_t data_size);
    int setFaceTexture(int face, const uint8_t* rgba_data, uint32_t width, uint32_t height);
    int setFaceMaterial(int face, const char* material_json);
