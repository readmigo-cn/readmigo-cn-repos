...
bool SceneManager::loadModelFromMemory(const uint8_t* data, uint32_t size, BadgeLOD lod) {
    if (!initialized_ || !asset_loader_ || !data || size == 0) return false;

    // Unload existing model
    unloadModel();

    // Create asset directly from memory buffer (no file I/O)
    asset_ = asset_loader_->createAsset(data, size);
    if (!asset_) return false;
...
bool SceneManager::setFaceTexture(int face_index, const uint8_t* rgba_data, uint32_t width, uint32_t height) {
    if (face_index < 0 || face_index > 2) return false;
    if (!asset_ || !renderer_ || !rgba_data) return false;

    auto* engine = renderer_->filamentEngine();

    // Destroy previous texture for this face if exists
    if (face_textures_[face_index]) {
        engine->destroy(face_textures_[face_index]);
        face_textures_[face_index] = nullptr;
    }
    ...
    // Copy pixel data into a new buffer (Filament takes ownership via callback)
    size_t data_size = width * height * 4;
    auto* pixel_copy = new uint8_t[data_size];
    std::memcpy(pixel_copy, rgba_data, data_size);

    filament::Texture::PixelBufferDescriptor pbd(
        pixel_copy, data_size,
    ...
    face_textures_[face_index] = texture;

    // Apply texture to the matching primitive on all renderable entities
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        size_t primCount = rm.getPrimitiveCount(ri);
        if (static_cast<size_t>(face_index) >= primCount) continue;

        auto* mi = rm.getMaterialInstanceAt(ri, static_cast<size_t>(face_index));
        if (!mi) continue;
        ...
        try {
            ...
            try {
                mi->setParameter("baseColor", texture, sampler);
            } catch (...) {}
        }
    }
...
bool SceneManager::setFaceMaterial(int face_index, const MaterialParams& params) {
    if (face_index < 0 || face_index > 2) return false;
    if (!asset_ || !renderer_) return false;
...
void SceneManager::destroyFaceTextures() {
    if (!renderer_) return;
    auto* engine = renderer_->filamentEngine();
    if (!engine) return;
    ...
    for (int i = 0; i < 3; i++) {
        if (face_textures_[i]) {
            engine->destroy(face_textures_[i]);
            face_textures_[i] = nullptr;
        }
    }
}
