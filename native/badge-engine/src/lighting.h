...
public:
    ...
    // Sweep highlight
    bool isSweepEnabled() const { return sweep_enabled_; }
    float sweepBandCenter() const;  // current band center position
    float sweepSpeed() const { return sweep_speed_; }
    float sweepWidth() const { return sweep_width_; }
    float sweepFalloff() const { return sweep_falloff_; }
    float sweepIntensity() const { return sweep_intensity_; }
    float sweepClamp() const { return sweep_clamp_; }
...
private:
    ...
    // Sweep highlight (vertical band)
    bool sweep_enabled_ = false;
    float sweep_speed_ = 1.2f;
    float sweep_width_ = 0.4f;
    float sweep_falloff_ = 0.05f;
    float sweep_intensity_ = 0.6f;
    float sweep_clamp_ = 0.85f;
    float sweep_time_ = 0.0f;
