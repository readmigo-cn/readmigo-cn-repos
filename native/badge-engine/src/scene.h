...
public:
    ...
    bool loadModelFromMemory(const uint8_t* data, uint32_t size, BadgeLOD lod);
    void unloadModel();
    ...
    bool setFaceTexture(int face_index, const uint8_t* rgba_data, uint32_t width, uint32_t height);
    bool setFaceMaterial(int face_index, const MaterialParams& params);

private:
    filament::Texture* face_textures_[3] = {nullptr, nullptr, nullptr};
