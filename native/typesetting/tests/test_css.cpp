...
TEST(CSSTest, ParseVendorPrefixHyphens) {
    auto s1 = CSSStylesheet::parse("body { -webkit-hyphens: auto; }");
    ASSERT_GE(s1.rules.size(), 1);
    EXPECT_TRUE(s1.rules[0].properties.hyphens.value_or(false));

    auto s2 = CSSStylesheet::parse("h1 { -moz-hyphens: none; }");
    ASSERT_GE(s2.rules.size(), 1);
    EXPECT_FALSE(s2.rules[0].properties.hyphens.value_or(true));

    auto s3 = CSSStylesheet::parse("p { -epub-hyphens: auto; }");
    ASSERT_GE(s3.rules.size(), 1);
    EXPECT_TRUE(s3.rules[0].properties.hyphens.value_or(false));

    auto s4 = CSSStylesheet::parse("body { adobe-hyphenate: auto; }");
    ASSERT_GE(s4.rules.size(), 1);
    EXPECT_TRUE(s4.rules[0].properties.hyphens.value_or(false));
}
...
TEST(CSSTest, ParseWidthPx) {
    auto sheet = CSSStylesheet::parse("hr { width: 48px; }");
    ASSERT_EQ(sheet.rules.size(), 1);
    EXPECT_TRUE(sheet.rules[0].properties.widthEm.has_value());
    EXPECT_FLOAT_EQ(sheet.rules[0].properties.widthEm.value(), 48.0f / 16.0f);
    EXPECT_FALSE(sheet.rules[0].properties.widthPercent.has_value());
}

TEST(CSSTest, ParseWidthEm) {
    auto sheet = CSSStylesheet::parse("div { width: 3em; }");
    ASSERT_EQ(sheet.rules.size(), 1);
    EXPECT_TRUE(sheet.rules[0].properties.widthEm.has_value());
    EXPECT_FLOAT_EQ(sheet.rules[0].properties.widthEm.value(), 3.0f);
}

TEST(CSSTest, ParseBreakInsideAvoid) {
    auto sheet = CSSStylesheet::parse("p { break-inside: avoid; }");
    ASSERT_EQ(sheet.rules.size(), 1);
    EXPECT_TRUE(sheet.rules[0].properties.breakInsideAvoid.has_value());
    EXPECT_TRUE(sheet.rules[0].properties.breakInsideAvoid.value());
}

TEST(CSSTest, ParsePageBreakInsideAvoid) {
    auto sheet = CSSStylesheet::parse("p { page-break-inside: avoid; }");
    ASSERT_EQ(sheet.rules.size(), 1);
    EXPECT_TRUE(sheet.rules[0].properties.breakInsideAvoid.has_value());
    EXPECT_TRUE(sheet.rules[0].properties.breakInsideAvoid.value());
}

TEST(CSSTest, ParseBreakAfterAvoid) {
    auto sheet = CSSStylesheet::parse("h2 { break-after: avoid; }");
    ASSERT_EQ(sheet.rules.size(), 1);
    EXPECT_TRUE(sheet.rules[0].properties.breakAfterAvoid.has_value());
    EXPECT_TRUE(sheet.rules[0].properties.breakAfterAvoid.value());
}

TEST(CSSTest, ParseOrphans) {
    auto sheet = CSSStylesheet::parse("p { orphans: 3; }");
    ASSERT_EQ(sheet.rules.size(), 1);
    EXPECT_TRUE(sheet.rules[0].properties.orphans.has_value());
    EXPECT_EQ(sheet.rules[0].properties.orphans.value(), 3);
}

TEST(CSSTest, ParseWidows) {
    auto sheet = CSSStylesheet::parse("p { widows: 4; }");
    ASSERT_EQ(sheet.rules.size(), 1);
    EXPECT_TRUE(sheet.rules[0].properties.widows.has_value());
    EXPECT_EQ(sheet.rules[0].properties.widows.value(), 4);
}

TEST(CSSTest, ParseNotPseudoClass) {
    auto sheet = CSSStylesheet::parse("p:not(:first-child) { text-indent: 1em; }");
    ASSERT_EQ(sheet.rules.size(), 1);
    auto& sel = sheet.rules[0].selector;
    EXPECT_EQ(sel.type, SelectorType::Element);
    EXPECT_EQ(sel.element, "p");
    ASSERT_NE(sel.negation, nullptr);
    EXPECT_EQ(sel.negation->type, SelectorType::PseudoFirstChild);
    EXPECT_EQ(sel.negation->pseudoClass, "first-child");
}

TEST(CSSTest, ParseNotClassSelector) {
    auto sheet = CSSStylesheet::parse("p:not(.special) { margin-top: 0; }");
    ASSERT_EQ(sheet.rules.size(), 1);
    auto& sel = sheet.rules[0].selector;
    EXPECT_EQ(sel.element, "p");
    ASSERT_NE(sel.negation, nullptr);
    EXPECT_EQ(sel.negation->type, SelectorType::Class);
    EXPECT_EQ(sel.negation->className, "special");
}

TEST(CSSTest, MergeNewProperties) {
    CSSProperties a;
    a.widthEm = 2.0f;
    a.breakInsideAvoid = true;
    a.orphans = 3;
    a.widows = 4;

    CSSProperties b;
    b.merge(a);
    EXPECT_TRUE(b.widthEm.has_value());
    EXPECT_FLOAT_EQ(b.widthEm.value(), 2.0f);
    EXPECT_TRUE(b.breakInsideAvoid.value());
    EXPECT_EQ(b.orphans.value(), 3);
    EXPECT_EQ(b.widows.value(), 4);
}
