...
TEST(StyleResolverTest, Heading5DefaultStyle) {
    CSSStylesheet sheet;
    StyleResolver resolver(sheet);
    ...
    EXPECT_EQ(resolved.blockStyles[0].alignment, TextAlignment::Center);
    EXPECT_FALSE(resolved.blockStyles[0].hyphens);
    EXPECT_NEAR(resolved.blockStyles[0].font.size, 18.0f * 0.83f, 0.1f);
}

TEST(StyleResolverTest, Heading6DefaultStyle) {
    CSSStylesheet sheet;
    StyleResolver resolver(sheet);
    ...
    EXPECT_EQ(resolved.blockStyles[0].alignment, TextAlignment::Center);
    EXPECT_FALSE(resolved.blockStyles[0].hyphens);
    EXPECT_NEAR(resolved.blockStyles[0].font.size, 18.0f * 0.67f, 0.1f);
}
...
TEST(StyleResolverTest, MultiLevelAdjacentSiblingMatches) {
    ...
    block.type = BlockType::Paragraph;
    block.htmlTag = "p";
    block.previousSiblings = {{"p"}, {"h2"}};  // [0]=immediate prev, [1]=before that
...
TEST(StyleResolverTest, MultiLevelAdjacentSiblingNoMatchWrongOrder) {
    ...
    block.type = BlockType::Paragraph;
    block.htmlTag = "p";
    block.previousSiblings = {{"h2"}};  // Only one previous sibling, need two
...
// =============================================================================
// MARK: - Multi-level Ancestor Matching (P2 fix)
// =============================================================================

TEST(StyleResolverTest, DescendantMatchesGrandparent) {
    // CSS: section.epigraph p { font-style: italic; }
    // HTML: <section class="epigraph"><blockquote><p>Text</p></blockquote></section>
    auto sheet = CSSStylesheet::parse("section.epigraph p { font-style: italic; }");
    StyleResolver resolver(sheet);
    ...
    // ancestors: [0]=blockquote (direct parent), [1]=section.epigraph (grandparent)
    block.ancestors = {
        {"blockquote", "", "", ""},
        {"section", "epigraph", "", ""}
    };
    block.inlines.push_back(InlineElement::plain("Epigraph text"));
...
TEST(StyleResolverTest, DescendantRejectsWrongAncestor) {
    auto sheet = CSSStylesheet::parse("section.epigraph p { font-style: italic; }");
    StyleResolver resolver(sheet);
    ...
    block.ancestors = {
        {"blockquote", "", "", ""},
        {"section", "chapter", "", ""}  // wrong class
    };
    block.inlines.push_back(InlineElement::plain("Normal text"));
    ...
    auto resolved = resolver.resolve({block}, userStyle);
    ASSERT_EQ(resolved.blockStyles.size(), 1);
    EXPECT_NE(resolved.blockStyles[0].font.style, FontStyle::Italic);
}

// =============================================================================
// MARK: - Adjacent Sibling className Check (P2 fix)
// =============================================================================

TEST(StyleResolverTest, AdjacentSiblingWithClassName) {
    // CSS: h2.title + p { text-indent: 0; }
    // Should only match p after h2 with class "title"
    auto sheet = CSSStylesheet::parse("h2.title + p { text-indent: 0; }");
    StyleResolver resolver(sheet);

    Block block;
    block.type = BlockType::Paragraph;
    block.htmlTag = "p";
    block.previousSiblings = {{"h2", "title"}};
    block.inlines.push_back(InlineElement::plain("Text"));
...
TEST(StyleResolverTest, AdjacentSiblingRejectsWrongClassName) {
    auto sheet = CSSStylesheet::parse("h2.title + p { text-indent: 0; }");
    StyleResolver resolver(sheet);
    ...
    block.type = BlockType::Paragraph;
    block.htmlTag = "p";
    block.previousSiblings = {{"h2", "subtitle"}};  // wrong class
    block.inlines.push_back(InlineElement::plain("Text"));
    ...
    auto resolved = resolver.resolve({block}, userStyle);
    ASSERT_EQ(resolved.blockStyles.size(), 1);
    EXPECT_FLOAT_EQ(resolved.blockStyles[0].textIndent, 18.0f);  // Default 1em indent, NOT overridden
}
...
// --- P2: width px/em, break-inside/after, orphans/widows ---

TEST(StyleResolverTest, WidthPxApplied) {
    auto sheet = CSSStylesheet::parse("hr { width: 48px; }");
    StyleResolver resolver(sheet);
    ...
    userStyle.font.size = 16.0f;
    auto resolved = resolver.resolve({block}, userStyle);
    // 48px / 16 = 3em → 3 * 16 = 48px
    EXPECT_FLOAT_EQ(resolved.blockStyles[0].widthPx, 48.0f);
}

TEST(StyleResolverTest, WidthEmApplied) {
    auto sheet = CSSStylesheet::parse("div { width: 5em; }");
    StyleResolver resolver(sheet);
    ...
    userStyle.font.size = 20.0f;
    auto resolved = resolver.resolve({block}, userStyle);
    EXPECT_FLOAT_EQ(resolved.blockStyles[0].widthPx, 100.0f);  // 5em * 20px
}

TEST(StyleResolverTest, BreakInsideAvoidApplied) {
    auto sheet = CSSStylesheet::parse("p { break-inside: avoid; }");
    StyleResolver resolver(sheet);
    ...
    userStyle.font.size = 16.0f;
    auto resolved = resolver.resolve({block}, userStyle);
    EXPECT_TRUE(resolved.blockStyles[0].breakInsideAvoid);
}

TEST(StyleResolverTest, BreakAfterAvoidApplied) {
    auto sheet = CSSStylesheet::parse("h2 { break-after: avoid; }");
    StyleResolver resolver(sheet);
    ...
    userStyle.font.size = 16.0f;
    auto resolved = resolver.resolve({block}, userStyle);
    EXPECT_TRUE(resolved.blockStyles[0].breakAfterAvoid);
}

TEST(StyleResolverTest, OrphansWidowsApplied) {
    auto sheet = CSSStylesheet::parse("p { orphans: 3; widows: 4; }");
    StyleResolver resolver(sheet);
    ...
    userStyle.font.size = 16.0f;
    auto resolved = resolver.resolve({block}, userStyle);
    EXPECT_EQ(resolved.blockStyles[0].orphans, 3);
    EXPECT_EQ(resolved.blockStyles[0].widows, 4);
}

TEST(StyleResolverTest, OrphansWidowsDefault) {
...
TEST(StyleResolverTest, NotFirstChildMatches) {
    // p:not(:first-child) should match p that is NOT first-child
    auto sheet = CSSStylesheet::parse("p:not(:first-child) { text-indent: 2em; }");
    StyleResolver resolver(sheet);

    Block block;
    block.type = BlockType::Paragraph;
    block.isFirstChild = false;
    block.previousSiblings = {{"p"}};

    Style userStyle;
    userStyle.font.size = 16.0f;
    auto resolved = resolver.resolve({block}, userStyle);
    EXPECT_FLOAT_EQ(resolved.blockStyles[0].textIndent, 2.0f * 16.0f);
}

TEST(StyleResolverTest, NotFirstChildRejectsFirstChild) {
    // p:not(:first-child) should NOT match p that IS first-child
    auto sheet = CSSStylesheet::parse("p:not(:first-child) { text-indent: 2em; }");
    StyleResolver resolver(sheet);
    ...
    userStyle.font.size = 16.0f;
    auto resolved = resolver.resolve({block}, userStyle);
    // Default paragraph indent is 1em (16px), not 2em (32px)
    EXPECT_FLOAT_EQ(resolved.blockStyles[0].textIndent, 1.0f * 16.0f);
}

TEST(StyleResolverTest, NotClassMatches) {
    // p:not(.special) should match p without class "special"
    auto sheet = CSSStylesheet::parse("p:not(.special) { margin-top: 2em; }");
    StyleResolver resolver(sheet);
    ...
    userStyle.font.size = 16.0f;
    auto resolved = resolver.resolve({block}, userStyle);
    EXPECT_FLOAT_EQ(resolved.blockStyles[0].marginTop, 2.0f * 16.0f);
}

TEST(StyleResolverTest, NotClassRejectsMatchingClass) {
    // p:not(.special) should NOT match p with class "special"
    auto sheet = CSSStylesheet::parse("p:not(.special) { margin-top: 2em; }");
    StyleResolver resolver(sheet);
