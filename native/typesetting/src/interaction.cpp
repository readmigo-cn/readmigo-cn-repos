...
void InteractionManager::setPlatform(std::shared_ptr<PlatformAdapter> platform) {
...
// ---------------------------------------------------------------------------
// getPreciseRectsForRange — pixel-precise highlight using CoreText glyph advances
// ---------------------------------------------------------------------------

std::vector<TextRect> InteractionManager::getPreciseRectsForRange(int pageIndex, int blockIndex,
                                                                    int charOffset, int charLength) const {
    // Fall back to linear interpolation if no platform adapter
    if (!platform_) {
        return getRectsForRange(pageIndex, blockIndex, charOffset, charLength);
    }
    ...
    for (const auto& line : page->lines) {
        for (const auto& run : line.runs) {
            ...
            if (run.wordSpacing > 0.01f) {
                // Justified text: CoreText doesn't know about wordSpacing,
                // so use linear interpolation which correctly uses run.width
                // (already includes justification).
                float startFrac = (run.charLength > 0)
                    ? static_cast<float>(overlapStart - runBlockStart) / run.charLength : 0;
                float endFrac = (run.charLength > 0)
                    ? static_cast<float>(overlapEnd - runBlockStart) / run.charLength : 1;
                rectX = run.x + startFrac * run.width;
                rectW = (endFrac - startFrac) * run.width;
            } else {
                // Non-justified text: use CoreText for precise glyph measurement
                float startX = platform_->measureOffset(run.text, run.font,
                                                         overlapStart - runBlockStart);
                float endX = platform_->measureOffset(run.text, run.font,
                                                       overlapEnd - runBlockStart);
                rectX = run.x + startX;
                rectW = endX - startX;
            }
...
// ---------------------------------------------------------------------------
// getRectsForRangeInkAware — like getRectsForRange but expands first/last rect
// to include the leading/trailing glyph ink overhang. Used by long-press
// sentence selection to avoid clipping wide-top capitals (T/V/W/A/S).
// ---------------------------------------------------------------------------

std::vector<TextRect> InteractionManager::getRectsForRangeInkAware(
    int pageIndex, int blockIndex, int charOffset, int charLength) const {
    auto rects = getRectsForRange(pageIndex, blockIndex, charOffset, charLength);
    if (rects.empty() || !platform_) return rects;
    ...
    // Locate the first run that contributed the leading rect and the last run
    // that contributed the trailing rect, in line/run iteration order — same
    // order as getRectsForRange so rects.front()/back() correspond.
    const TextRun* firstRun = nullptr;
    int firstRunBlockStart = 0;
    int firstOverlapStart = 0;
    const TextRun* lastRun = nullptr;
    int lastRunBlockStart = 0;
    int lastOverlapEnd = 0;

    for (const auto& line : page->lines) {
        for (const auto& run : line.runs) {
            if (run.blockIndex != blockIndex) continue;
            int runBlockStart = toBlockOffset(blocks_[blockIndex],
                                              run.inlineIndex, run.charOffset);
            int runBlockEnd = runBlockStart + run.charLength;
            int overlapStart = std::max(rangeStart, runBlockStart);
            int overlapEnd = std::min(rangeEnd, runBlockEnd);
            if (overlapStart >= overlapEnd) continue;
            if (firstRun == nullptr) {
                firstRun = &run;
                firstRunBlockStart = runBlockStart;
                firstOverlapStart = overlapStart;
            }
            lastRun = &run;
            lastRunBlockStart = runBlockStart;
            lastOverlapEnd = overlapEnd;
        }
    }

    if (firstRun != nullptr) {
        int byteOff = firstOverlapStart - firstRunBlockStart;
        int byteLen = std::min(8, firstRun->charLength - byteOff);
        if (byteLen > 0) {
            InkOverhang oh = platform_->getInkOverhang(
                firstRun->text, firstRun->font, byteOff, byteLen);
            if (oh.left > 0.0f) {
                rects.front().x -= oh.left;
                rects.front().width += oh.left;
            }
        }
    }
    if (lastRun != nullptr) {
        // Measure trailing ink overhang of the last few bytes.
        int byteEnd = lastOverlapEnd - lastRunBlockStart;
        int byteLen = std::min(8, byteEnd);
        int byteOff = byteEnd - byteLen;
        if (byteLen > 0 && byteOff >= 0) {
            InkOverhang oh = platform_->getInkOverhang(
                lastRun->text, lastRun->font, byteOff, byteLen);
            if (oh.right > 0.0f) {
                rects.back().width += oh.right;
            }
        }
    }
