...
/// Collapse whitespace (HTML white-space:normal behavior):
/// Normalize runs of whitespace to a single space, preserving
/// leading/trailing spaces for inline element boundary handling.
...
BlockType tagToBlockType(const std::string& tagName) {
    ...
    if (tagName == "h5") return BlockType::Heading5;
    if (tagName == "h6") return BlockType::Heading6;
...
std::vector<Block> parseHTML(const std::string& html) {
    ...
    // Track recent siblings at each nesting level for adjacent sibling matching
    // Key: parentStack depth, Value: recent siblings (most recent first, max 5)
    std::vector<std::vector<SiblingInfo>> siblingHistoryAtDepth;

    // Poem paragraph mode: true when inside a <p> within a poem blockquote.
    // In this mode each <span> becomes its own block instead of an inline run.
    bool inPoemParagraph = false;

    // Helper: check if the parent stack contains a poem blockquote ancestor
    auto isInPoemContext = [&]() -> bool {
        for (const auto& p : parentStack) {
            if (p.epubType.find("z3998:poem") != std::string::npos ||
                p.className.find("z3998-poem") != std::string::npos ||
                p.className.find("z3998:poem") != std::string::npos) {
                return true;
            }
        }
    ...
    auto populateBlockMeta = [&](Block& block, const std::string& tagName, const Tag& tag) {
        ...
        block.dataParent = tag.getAttribute("data-parent");
        block.role = tag.getAttribute("role");
        ...
        // Populate ancestor chain from parentStack (back=direct parent, front=root)
        block.ancestors.clear();
        for (int i = static_cast<int>(parentStack.size()) - 1; i >= 0; --i) {
            AncestorInfo ai;
            ai.tag = parentStack[i].tag;
            ai.className = parentStack[i].className;
            ai.epubType = parentStack[i].epubType;
            ai.id = parentStack[i].id;
            block.ancestors.push_back(ai);
        }

        size_t depth = parentStack.size();
        if (depth < siblingHistoryAtDepth.size()) {
            block.previousSiblings = siblingHistoryAtDepth[depth];
        }
        // Update sibling history - insert new sibling at front, limit to 5
        if (depth >= siblingHistoryAtDepth.size()) {
            siblingHistoryAtDepth.resize(depth + 1);
        }
        SiblingInfo newSib;
        newSib.tag = tagName;
        newSib.className = tag.getAttribute("class");
        siblingHistoryAtDepth[depth].insert(siblingHistoryAtDepth[depth].begin(), newSib);
        if (siblingHistoryAtDepth[depth].size() > 5) {
            siblingHistoryAtDepth[depth].pop_back();
        }
    };

    // Helper: trim trailing whitespace from last inline when finalizing a block
    auto trimBlockTrailingSpace = [](Block& block) {
        if (block.inlines.empty()) return;
        auto& last = block.inlines.back().text;
        if (!last.empty() && last.back() == ' ') {
            last.pop_back();
        }
        // Remove the inline if it became empty after trimming
        if (block.inlines.back().text.empty()) {
            block.inlines.pop_back();
        }
    };
        ...
        if (html[pos] == '<') {
            ...
            if (tag.name == "table" && !tag.isClosing) {
                if (inBlock && !currentBlock.inlines.empty()) {
                    trimBlockTrailingSpace(currentBlock);
            ...
            if (tag.name == "br") {
                // In poem paragraphs <br/> is display:none (SE CSS); ignore it
                if (!inPoemParagraph && inBlock) {
            ...
            // Poem paragraph: intercept <p> open inside a poem blockquote
            if (tag.name == "p" && !tag.isClosing && isInPoemContext()) {
                if (inBlock && !currentBlock.inlines.empty()) {
                    trimBlockTrailingSpace(currentBlock);
                    blocks.push_back(currentBlock);
                    currentBlock = Block{};
                ...
                // Push <p> as a container so child blocks get correct parent/ancestor info
                ParentInfo pi;
                pi.tag = "p";
                pi.className = tag.getAttribute("class");
                pi.epubType = tag.getAttribute("epub:type");
                ...
                siblingHistoryAtDepth[parentStack.size()].clear();
                inPoemParagraph = true;
                pos = nextPos;
                continue;
            }

            // Poem paragraph: intercept </p> close
            if (tag.name == "p" && tag.isClosing && inPoemParagraph) {
                if (inBlock && !currentBlock.inlines.empty()) {
                    trimBlockTrailingSpace(currentBlock);
                    blocks.push_back(currentBlock);
                    currentBlock = Block{};
                ...
                if (!parentStack.empty() && parentStack.back().tag == "p") {
                    parentStack.pop_back();
                }
                inPoemParagraph = false;
            ...
            // Poem paragraph: each <span> opens a new block
            if (inPoemParagraph && tag.name == "span" && !tag.isClosing) {
                if (inBlock && !currentBlock.inlines.empty()) {
                    trimBlockTrailingSpace(currentBlock);
                    blocks.push_back(currentBlock);
                }
                currentBlock = Block{};
                currentBlock.type = BlockType::Paragraph;
                currentBlock.htmlTag = "p";

                // Derive indent level from span class: "i1" → poem-line-i1 → margin 1em
                auto spanCls = tag.getAttribute("class");
                int indentLevel = 0;
                if (spanCls.size() >= 2 && spanCls[0] == 'i') {
                    try { indentLevel = std::stoi(spanCls.substr(1)); } catch (...) {}
                }
                currentBlock.className = "poem-line-i" + std::to_string(indentLevel);

                // Parent/ancestor info from current parentStack
                if (!parentStack.empty()) {
                    auto& parent = parentStack.back();
                    currentBlock.parentTag = parent.tag;
                    currentBlock.parentClassName = parent.className;
                    currentBlock.parentEpubType = parent.epubType;
                    currentBlock.parentId = parent.id;
                    currentBlock.isFirstChild = (parent.childBlockCount == 0);
                    parent.childBlockCount++;
                }
                currentBlock.ancestors.clear();
                for (int i = static_cast<int>(parentStack.size()) - 1; i >= 0; --i) {
                    AncestorInfo ai;
                    ai.tag = parentStack[i].tag;
                    ai.className = parentStack[i].className;
                    ai.epubType = parentStack[i].epubType;
                    ai.id = parentStack[i].id;
                    currentBlock.ancestors.push_back(ai);
                }
            ...
            // Poem paragraph: </span> closes the current line block
            if (inPoemParagraph && tag.name == "span" && tag.isClosing) {
                // block will be flushed at next <span> open or </p>
                currentInline = InlineType::Text;
                currentInlineRaw.clear();
                currentInlineTag.clear();
                pos = nextPos;
            ...
            if (!tag.isClosing) {
                ...
                // <span> with class-based styling (Standard Ebooks patterns)
                if (tag.name == "span" || tag.name == "abbr" || tag.name == "small" ||
                    tag.name == "sub" || tag.name == "sup") {
                    auto cls = tag.getAttribute("class");
                    if (cls == "i" || cls == "italic") {
                        inType = InlineType::Italic;
                    } else if (cls == "b" || cls == "bold") {
                        inType = InlineType::Bold;
                    }
                    // Other classes (sc, etc.) stay as Text — CSS rules handle them
                } else if (inType == InlineType::Text && tag.name != "b" && tag.name != "strong" &&
            ...
            // Skip whitespace-only text when not inside a block (inter-block whitespace)
            if (collapsed.empty() || (collapsed == " " && !inBlock)) {
                pos = nextTag;
                continue;
            }

            // Trim leading space at start of a block (first inline element)
            if (inBlock && currentBlock.inlines.empty()) {
                if (!collapsed.empty() && collapsed.front() == ' ') {
                    collapsed.erase(0, 1);
                }
            }
            if (!inBlock && !collapsed.empty() && collapsed.front() == ' ') {
                collapsed.erase(0, 1);
            }
    ...
    // Log each block's type and content preview for debugging
    for (size_t bi = 0; bi < blocks.size(); ++bi) {
        const auto& b = blocks[bi];
        const char* typeName = "?";
        switch (b.type) {
            case BlockType::Paragraph:     typeName = "P"; break;
            case BlockType::Heading1:      typeName = "H1"; break;
            case BlockType::Heading2:      typeName = "H2"; break;
            case BlockType::Heading3:      typeName = "H3"; break;
            case BlockType::Heading4:      typeName = "H4"; break;
            case BlockType::Heading5:      typeName = "H5"; break;
            case BlockType::Heading6:      typeName = "H6"; break;
            case BlockType::Blockquote:    typeName = "BQ"; break;
            case BlockType::CodeBlock:     typeName = "CODE"; break;
            case BlockType::Image:         typeName = "IMG"; break;
            case BlockType::HorizontalRule:typeName = "HR"; break;
            case BlockType::ListItem:      typeName = "LI"; break;
            case BlockType::Figcaption:    typeName = "FIG"; break;
            case BlockType::Table:         typeName = "TBL"; break;
        }
        std::string preview = b.plainText();
        if (preview.size() > 120) preview = preview.substr(0, 120) + "...";
        TS_LOGI("parseHTML: block[%zu] type=%s tag=<%s> class='%s' parent=<%s> text='%s'",
                bi, typeName, b.htmlTag.c_str(), b.className.c_str(),
                b.parentTag.c_str(), preview.c_str());
    }
    ...
    for (const auto& b : blocks) {
        switch (b.type) {
            ...
            case BlockType::Heading6:      ++nH; break;
