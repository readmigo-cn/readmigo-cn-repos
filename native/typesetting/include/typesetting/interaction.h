...
public:
    ...
    /// Set the platform adapter for precise text measurement
    void setPlatform(std::shared_ptr<PlatformAdapter> platform);
    ...
    /// Precise character range → visual rectangles (uses CoreText glyph advances).
    /// Used for TTS word-level highlighting where pixel-precision matters.
    std::vector<TextRect> getPreciseRectsForRange(int pageIndex, int blockIndex,
                                                   int charOffset, int charLength) const;

    /// Like getRectsForRange but expands the leading rect's left edge and the
    /// trailing rect's right edge to cover the glyphs' ink overhang
    /// (left-side bearing of capitals like T/V/W/A/S that protrude past the
    /// typographic origin). Used by long-press sentence selection so the first
    /// letter is not visually clipped.
