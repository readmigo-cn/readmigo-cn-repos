...
enum : uint32_t {
    ...
    kImpWidthEm         = 1u << 24,
    kImpBreakInside     = 1u << 25,
    kImpBreakAfter      = 1u << 26,
    kImpOrphans         = 1u << 27,
    kImpWidows          = 1u << 28,
    kImpBreakBefore     = 1u << 29,
    kImpPosition        = 1u << 30,
    kImpPaddingRight    = 1u << 31,
};

/// Extended important flags (importantFlags2)
enum : uint32_t {
    kImp2TextDecoration  = 1u << 0,
    kImp2BorderCollapse  = 1u << 1,
    kImp2BorderLeftWidth = 1u << 2,
    kImp2ListStyle       = 1u << 3,
    kImp2FontFamily      = 1u << 4,
};
...
struct CSSSelector {
    ...
    std::shared_ptr<CSSSelector> negation;         // For :not() pseudo-class
...
struct CSSProperties {
    ...
    std::optional<float> paddingRight;    // in em units
    ...
    std::optional<float> maxWidthEm;       // max-width in em units
    ...
    std::optional<float> widthEm;          // width in em units (for px/em CSS values)
    std::optional<bool> breakInsideAvoid;  // break-inside: avoid
    std::optional<bool> breakAfterAvoid;   // break-after: avoid
    std::optional<int> orphans;            // orphans count
    std::optional<int> widows;             // widows count
    std::optional<bool> verticalCenter;    // justify-content: center (flex vertical centering)
    std::optional<bool> breakBeforePage;   // break-before: page
    std::optional<bool> breakAfterPage;    // break-after: page (always)
    std::optional<bool> positionAbsolute;  // position: absolute (treat as hidden)
    std::optional<std::string> textDecoration;  // "underline", "none"
    std::optional<bool> borderCollapse;    // border-collapse: collapse
    std::optional<float> borderLeftWidth;  // in px
    std::optional<bool> listStyleNone;     // list-style: none / list-style-type: none
    std::optional<std::string> fontFamily; // "monospace", "serif", "sans-serif"

    uint32_t importantFlags = 0;   // Bitfield of kImp* flags
    uint32_t importantFlags2 = 0;  // Bitfield of kImp2* flags
