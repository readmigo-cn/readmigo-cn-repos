...
/// Ancestor element info for CSS descendant selector matching
struct AncestorInfo {
    std::string tag;
...
/// Sibling element info for CSS adjacent sibling selector matching
struct SiblingInfo {
    std::string tag;
    std::string className;
};
...
struct Block {
    ...
    std::vector<SiblingInfo> previousSiblings;      // Previous siblings [0]=immediate, [1]=before that, etc.
    std::vector<AncestorInfo> ancestors;             // Ancestor chain [0]=direct parent, [1]=grandparent, etc.
    std::string id;                       // HTML id attribute
    std::string dataParent;               // data-parent attribute
    std::string role;                     // role attribute
