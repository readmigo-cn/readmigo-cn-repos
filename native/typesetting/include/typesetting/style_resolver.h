...
private:
    ...
    /// Apply only inheritable CSS properties (text-align, font-*, hyphens, etc.)
    /// Used for CSS inheritance from ancestor container elements.
    void applyInheritableProperties(const CSSProperties& props, BlockComputedStyle& style, float baseFontSize, bool importantOnly = false) const;

    /// Apply user Style overrides as final layer
    void applyUserOverrides(BlockComputedStyle& style, const Style& userStyle, const Block& block, bool cssFontSizeSet, bool cssLineHeightSet, bool cssTextAlignSet, bool cssFontFamilySet) const;
